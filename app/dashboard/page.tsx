...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const results = geoNames
       .filter((nome: any) => nome?.toLowerCase().includes(q))
  ...
  return (
    ...
    <main className="flex h-screen w-full bg-[#050505] text-slate-200 overflow-hidden font-sans">
      ...
      {/* Hamburger mobile */}
      <button
        className="fixed top-4 left-4 z-50 md:hidden bg-[#0a0a0a] border border-slate-800 rounded-xl p-2 text-slate-400 hover:text-slate-200 transition-colors"
      ...
      <div
        ...
        className={`${isSidebarExpanded ? "w-[550px]" : "w-[320px]"} transition-all duration-300 ease-in-out border-r border-slate-800/50 flex flex-col bg-[#0a0a0a] z-20 shadow-2xl relative shrink-0 fixed md:relative h-full ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
